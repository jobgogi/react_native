...
export const runPracticeDayjs = () => {
  ...
  console.log(
    '3. set hour',
    dayjs(now).set('hour', 10).format('YYYY.MM.DD hh:mm:ss a A')
  );
  console.log('4. get year', dayjs(now).get('year'));
  console.log('5. get date', dayjs(now).get('date'));
  console.log('6. get month', dayjs(now).get('month')); // 0 ~ 11 월로 되어 있음
  console.log('7. get day', dayjs(now).get('day')); // 요일을 가져와라 0: 일 ~ 6: 토
  console.log('8. get second', dayjs(now).get('second'));
  console.log(
    '9. add hour',
    dayjs(now).add(3, 'hour').format('YYYY.MM.DD hh:mm:ss a A')
  );
  console.log(
    '10. subtract hour',
    dayjs(now).subtract(3, 'hour').format('YYYY.MM.DD hh:mm:ss a A')
  );
  ...
  console.log('24, diff minute a, b', dayjs(bDate).diff(aDate, 'minute'));
};
